...
import 'package:zairok/config/api_secrets.dart';
...
''';
  ...
  /// ✅ Build the final image generation URL using NEW Pollinations API endpoint
  /// NEW ENDPOINT: https://gen.pollinations.ai/image/{prompt}?model=flux
static Uri buildImageGenUri({
  ...
  if (customNegativePrompt != null && customNegativePrompt.isNotEmpty) {
    queryParams['negative_prompt'] = customNegativePrompt;
  } else {
    queryParams['negative_prompt'] = NegativePrompt.trim();
  }
  ...
  // ✅ Build and return the final image generation URI using NEW endpoint
  return Uri.parse(
    "https://gen.pollinations.ai/image/$encodedPrompt?$queryString"
  );
}
...
/// 🖼️ Stores generated image data (bytes + metadata)
...
void generateImage() async {
  ...
  setState(() {
    isGenerating = true;
    generatedImages.clear(); // Clear previous results
  });
  ...
  final apiKey = await PollinationsApiConfig.getApiKey();
  ...
  for (final style in styleNames) {
    ...
    // 🎲 Generate unique seed for each style to get different outputs
    final randomSeed = (Random().nextInt(900000000) + 100000000).toString();
    final useSeed = seedInput ?? randomSeed;
    ...
    try {
      // 🖼️ Fetch binary image data from the new endpoint
      final response = await http.get(uri, headers: headers)
          .timeout(const Duration(seconds: 90));

      if (response.statusCode == 200 && response.bodyBytes.isNotEmpty) {
        // ✅ Success - store image bytes
        setState(() {
          generatedImages.add({
        ...
        // ❌ Failed - add error placeholder
        setState(() {
          generatedImages.add({
            'bytes': null,
            'style': style,
            'error': 'Failed to generate (${response.statusCode})',
            'url': uri.toString(),
            'seed': useSeed,
          });
        });
...
TextButton.icon(
  ...
  if (generatedImages.isEmpty && !isGenerating) return const SizedBox.shrink();
  ...
  return Padding(
    ...
    child: GridView.builder(
      ...
      itemBuilder: (context, index) {
        final imageData = generatedImages[index];
        final style = imageData['style'] as String;
        final bytes = imageData['bytes'] as Uint8List?;
        final error = imageData['error'] as String?;
        // URL stored for history/debugging purposes
            ...
            children: [
                ...
                child: ClipRRect(
                  ...
                  child: bytes != null
                      ? Image.memory(
                          ...
                          fit: BoxFit.cover,
                          errorBuilder: (_, __, ___) => _buildImageError(error ?? 'Failed to load'),
                        )
                      : _buildImageError(error ?? 'Generation failed'),
                ),
              ),
              Padding(
              ...
              if (bytes != null)
                TextButton.icon(
                  ...
                  onPressed: () {
                    _saveImageBytesToGallery(bytes, style);
                  },
                ),
              const SizedBox(height: 6),
            ],
...
/// 🖼️ Error widget for failed image generation
...
Future<void> _saveImageBytesToGallery(Uint8List bytes, String styleName) async {
  // 🔐 Request permissions
  bool granted = false;
  ...
  if (!granted) {
    showCuteSnackBar(context, "❌ Storage permission denied", bg: Colors.red.shade600);
    return;
  }

  // 🕒 Auto-generate file name
  final now = DateTime.now();
  final formattedName = "ZairokAI_${styleName}_${now.year}${_twoDigit(now.month)}${_twoDigit(now.day)}_${_twoDigit(now.hour)}${_twoDigit(now.minute)}${_twoDigit(now.second)}";
  ...
  try {
    ...
    if (result.isSuccess) {
      showCuteSnackBar(context, "✅ Saved as $formattedName.jpg", bg: Colors.green.shade600);
    } else {
      showCuteSnackBar(context, "❌ Failed to save image", bg: Colors.red.shade600);
    }
  } catch (e) {
    showCuteSnackBar(context, "❌ Error saving: $e", bg: Colors.red.shade700);
  }
}

String _twoDigit(int val) => val.toString().padLeft(2, '0');
}
...
Future<void> _sendPrompt(String prompt, {bool isRegenerate = false}) async {
  ...
  // Build message list for the new API format
  final apiMessages = <Map<String, String>>[];
  const maxPairs = 6;
  int pairsUsed = 0;

  // Collect conversation history for context
  for (int i = 0; i < newMessages.length && pairsUsed < maxPairs; i++) {
    ...
    if (role != null && text != null && text.isNotEmpty) {
      if (role == 'user') {
        ...
        apiMessages.add({'role': 'assistant', 'content': text});
        pairsUsed++;
      }
    }
  }
  ...
  // Add current prompt
  apiMessages.add({'role': 'user', 'content': promptToSend});

  // NEW ENDPOINT: https://gen.pollinations.ai/v1/chat/completions
  final uri = Uri.parse('https://gen.pollinations.ai/v1/chat/completions');

  try {
    // Build request body for the new API format
    final requestBody = jsonEncode({
      'model': currentModel,
      'messages': apiMessages,
    });

    // Get API key from secure storage (optional - Pollinations may work without key)
    final apiKey = await PollinationsApiConfig.getApiKey();
    ...
    if (response.statusCode == 200 && response.body.isNotEmpty) {
      ...
      if (data['choices'] != null && data['choices'].isNotEmpty) {
        ...
        fullReply = choice['message']?['content']?.toString() ?? '';
      } else if (data['content'] != null) {
        // Fallback for alternative response format
        fullReply = data['content'].toString();
      }

      if (fullReply.isEmpty) {
        fullReply = '⚠️ Empty response received. Please try again.';
      }
      ...
      _addBotReply("🔐 API key error. Please check your API configuration.");
    } else if (response.statusCode == 429) {
      _addBotReply("⏳ Rate limit reached. Please wait and try again.");
    } else {
      _addBotReply("⚠️ Server Busy (${response.statusCode}). Please try again or switch models.");
    }
  } on TimeoutException {
    _addBotReply("⏱️ Request timed out. Please check your connection and try again.");
  } catch (e) {
...
/// 🔑 Initialize API key securely from environment or secure config
Future<void> _initializeApiKey() async {
  // Priority 1: Check compile-time environment variable
  const apiKeyFromEnv = String.fromEnvironment('POLLINATIONS_API_KEY');
  if (apiKeyFromEnv.isNotEmpty) {
    await PollinationsApiConfig.setApiKey(apiKeyFromEnv);
    return;
  }
  
  // Priority 2: Check if already stored in secure storage
  final hasKey = await PollinationsApiConfig.hasApiKey();
  if (hasKey) {
    return; // Already configured
  }
  
  // Priority 3: Load from secure config (imported from api_secrets.dart)
  try {
    final configKey = await _loadSecureApiKey();
    if (configKey != null && configKey.isNotEmpty) {
      await PollinationsApiConfig.setApiKey(configKey);
    }
  } catch (_) {
    // Config file not found or invalid - user will need to set key manually
  }
}

/// Load API key from secure configuration
Future<String?> _loadSecureApiKey() async {
  // This loads from the secure config file (lib/config/api_secrets.dart)
  // The file is gitignored and contains only the API key
  try {
    return ApiSecrets.pollinationsApiKey;
  } catch (_) {
    return null;
  }
}
...
void main() async {
  ...
  // 🔑 Initialize Pollinations API key securely
  // Key is loaded from compile-time environment variable OR secure config
  // Run with: flutter run --dart-define=POLLINATIONS_API_KEY=your_key
